...
//        String timeStr = "2020-12-01 14:30:00";
        ...
        if (now.isAfter(meetingTimeInBeijing.toLocalDateTime())) {
            LocalDateTime tomorrow = now.plusDays(1);
            int newDayOfYear = tomorrow.getDayOfYear();

            LocalDateTime meetingTimeInChicago = meetingTimeInBeijing.withDayOfYear(newDayOfYear).withZoneSameInstant(CHICAGO_ZONE).toLocalDateTime();

            // 格式化新会议时间
            String showTimeStr = formatter.format(meetingTimeInChicago);
            System.out.println("The meeting time is ：" + showTimeStr + "(Chicago Time)");
        } else {
            ...
            System.out.println("The meeting has not start yet, There is still " + period.getDays() + " day before the meeting starts");
            System.out.println("The meeting time is ：" + showTimeStr + "(Chicago Time)");
        }
    }
}
